...
import com.zcom.zcompicturebackend.model.dto.space.analyze.*;
...
import com.zcom.zcompicturebackend.model.vo.space.analyze.*;
import com.zcom.zcompicturebackend.service.PictureService;
import com.zcom.zcompicturebackend.service.SpaceAnalyzeService;
...
/*        Map<String, Long> tagCountMap = tagsJsonList.stream()
        ...
        // 改进, 处理tags字段的值为空字符串的情况
        Map<String, Long> tagCountMap = tagsJsonList.stream()
                .flatMap(tagsJson -> {
                    try {
                        if (tagsJson == null || tagsJson.trim().isEmpty() || !tagsJson.trim().startsWith("[")) {
                            return Stream.empty(); // 跳过无效或空数据
                        }
                        return JSONUtil.toList(tagsJson, String.class).stream();
                    } catch (JSONException e) {
                        return Stream.empty(); // 跳过异常数据
                    }
                })
                .filter(ObjUtil::isNotNull) // 过滤 null 标签
                .collect(Collectors.groupingBy(tag -> tag, Collectors.counting()));
