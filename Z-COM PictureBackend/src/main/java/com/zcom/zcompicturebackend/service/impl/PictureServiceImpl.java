...
import com.zcom.zcompicturebackend.api.ailiyunai.AliYunAiApi;
import com.zcom.zcompicturebackend.api.ailiyunai.model.CreateOutPaintingTaskRequest;
import com.zcom.zcompicturebackend.api.ailiyunai.model.CreateOutPaintingTaskResponse;
...
import com.zcom.zcompicturebackend.manager.FileManager;
import com.zcom.zcompicturebackend.manager.upload.FilePictureUpload;
import com.zcom.zcompicturebackend.manager.upload.PictureUploadTemplate;
import com.zcom.zcompicturebackend.manager.upload.UrlPictureUpload;
import com.zcom.zcompicturebackend.mapper.PictureMapper;
...
import com.zcom.zcompicturebackend.utils.ColorSimilarUtils;
import com.zcom.zcompicturebackend.utils.ColorTransformUtils;
 ...
 * @createDate 2025-05-20 10:02:35
 */
@Service
@Slf4j
...
/*            // 仅本人或者管理员可编辑图片
        ...
        transactionTemplate.execute(status -> {
            ...
            if (finalSpaceId != null) {
                // 更新空间的使用额度
                // .setSql(), 执行一个计算, 把当前使用的空间 = 原本使用的空间 + 当前图片占用的空间
                boolean update = spaceService.lambdaUpdate()
                        .eq(Space::getId, finalSpaceId)
                        .setSql("totalSize = totalSize + " + picture.getPicSize())
                        .setSql("totalCount = totalCount + 1")
                        .update();
                ThrowUtils.throwIf(!update, ErrorCode.OPERATION_ERROR, "额度更失败");
            }
            return picture;
        });
    ...
    /**
     * 对单条数据进行脱敏
     * 但每条数据都要查询一下用户表, 性能消耗大
     * 可以先取所有的userid, 对userid进行去重
     * @param picture
     * @param request
     * @return
     */
    ...
    public void clearPictureFile(Picture oldPicture) {
        ...
        // 删除缩略图
        String thumbnaiUrl = oldPicture.getThumbnailUrl();
        ...
        transactionTemplate.execute(status -> {
            ...
            if (spaceId != null) {
                boolean update = spaceService.lambdaUpdate()
                        ...
                        .setSql("totalCount = totalCount - 1")
                        .update();
                ThrowUtils.throwIf(!update, ErrorCode.OPERATION_ERROR, "额度更新失败失败");
            }
            return true;
        });
        // 异步清理文件
