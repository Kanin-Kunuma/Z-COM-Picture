...
//        System.out.println("解析 tags: " + (tagsStr == null ? "null" : tagsStr)); // 调试
        if (StrUtil.isBlank(tagsStr)) {
            pictureVO.setTags(Collections.emptyList());
        } else {
            try {
                pictureVO.setTags(JSONUtil.toList(tagsStr, String.class));
            } catch (JSONException e) {
                System.out.println("tags 解析失败: " + tagsStr + ", 错误: " + e.getMessage());
                pictureVO.setTags(Collections.emptyList());
            }
        }
        return pictureVO;
